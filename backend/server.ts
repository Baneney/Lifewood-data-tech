...
app.post('/api/hired-or-rejected', async (req, res) => {
  ...
  try {
    await brevo.transactionalEmails.sendTransacEmail({
      ...
      sender: { name: "Lifewood HR", email: process.env.VERIFIED_SENDER_EMAIL! },
      to: [{ email, name }],
      htmlContent: `
      <!DOCTYPE html>
      <html>
    ...
    console.error("Brevo Hired/Rejected Error:", error);
...
app.post('/api/send-confirmation', async (req, res) => {
  ...
  try {
    ...
    console.error("Brevo Confirmation Error:", error);
...
app.post('/api/contact-inquiry', async (req, res) => {
  ...
  try {
    await brevo.transactionalEmails.sendTransacEmail({
      ...
      sender: { name: `${name} (${email})`, email: process.env.VERIFIED_SENDER_EMAIL! },
      to: [{ email: process.env.VERIFIED_SENDER_EMAIL! }],
      replyTo: { email },
      htmlContent: `
      <!DOCTYPE html>
      <html>
    ...
    console.error('Brevo Inquiry Error:', error);
    res.status(500).json({ error: 'Mail error' });
  }
});
