...
export default function Highlight({ img, tag, title, desc, to, reverse }: HighlightProps) {
  ...
  return (
    ...
    >
      {/* Image side */}
      <div className="relative w-full md:w-1/2 h-72 md:h-auto self-stretch overflow-hidden flex bg-[#034E34]/10">
        <img
          src={img}
          alt={title}
      ...
      <div
        ...
        border-l border-[#034E34]/10 dark:border-white/10
        ${
          hovered
            ? "bg-[#034E34] dark:bg-[#034E34]/70"
            : "bg-[#f0f7f3] dark:bg-[#133020]"
        }`}
      >
        <div
          className={`absolute -top-10 -right-10 w-40 h-40 rounded-full bg-[#FFB347]/20 transition-transform duration-700 ${hovered ? "scale-150" : "scale-0"}`}
        />
        <div
          className={`absolute -bottom-6 -left-6 w-28 h-28 rounded-full bg-[#FFB347]/10 transition-transform duration-700 delay-100 ${hovered ? "scale-150" : "scale-0"}`}
        />

        <span
          className={`text-xs font-semibold uppercase tracking-widest transition-colors duration-300 ${hovered ? "text-[#FFB347]" : "text-[#034E34]/50 dark:text-white/40"}`}
        >
          {tag}
        </span>

        <h3
          className={`mt-3 text-2xl md:text-3xl font-bold leading-snug transition-colors duration-300 text-[#034E34] dark:text-white group-hover:text-white `}
        >
          {title}
        </h3>

        <p
          className={`mt-4 leading-relaxed transition-colors duration-300 text-[#034E34]/70 dark:text-gray-400 group-hover:text-white/80`}
        >
          {desc}
        </p>
